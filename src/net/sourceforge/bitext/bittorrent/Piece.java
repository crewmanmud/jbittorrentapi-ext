...
public class Piece {
    ...
    public Piece(final int index, int length, int blockSize, byte[] sha1){
     ...
     */
    public Piece(final int index, int length, int blockSize, byte[] sha1, TreeMap<Integer, Integer> m) {
        this.index = index;
        this.length = length;
    ...
    public TreeMap<Integer, Integer> getFileAndOffset(){
    ...
    public synchronized byte[] data(){
        ...
        for (int i : pieceBlock.keySet())
            data = Utils.concat(data, this.pieceBlock.get(i));
        return data;
    }
    ...
    public synchronized boolean verify(){
    	String dataHash = Utils.byteArrayToByteString(Utils.hash(this.data()));
    	String pieceHash = Utils.byteArrayToByteString(this.sha1);
    	//System.out.println("Downloaded data: " + dataHash + " -> Piece hash: " + pieceHash);
        return dataHash.matches(pieceHash);
    }
    ...
    public synchronized String toString(){
        ...
        if(this.filesAndoffset.size() > 1)
            s += "s";
        for(Iterator<Integer> it = this.filesAndoffset.keySet().iterator(); it.hasNext();){
