...
public class MessageSender extends Thread {
    ...
    private long lmst = 0; // Last message send time.
    private boolean run = true;
    private final EventListenerList listeners = new EventListenerList();
    
    private static final long KEEPALIVE_INTERVAL = 120000L;
    ...
    public void run() {
        ...
        try {
            while (this.run) {
                ...
                out = outgoingMessage.poll(KEEPALIVE_INTERVAL, TimeUnit.MILLISECONDS);
                if(out != null){
                	byte[] message = out.generate();
                    os.write(message);
                    this.lmst = System.currentTimeMillis();
                    out = null;
                }else if(this.run){
