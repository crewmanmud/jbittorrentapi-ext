...
public class PeerUpdater extends Thread {
    ...
    public void run() {
        ...
        while (!this.end) {
            ...
            if (peerList != null) {
                ...
                try {
                    synchronized (b) {
                    	// Error contacting tracker... Wait to retry!
                        b.wait(10000);
                    }
                } catch (InterruptedException ie) {
                	// Trying to shutdown?
	...
	public synchronized LinkedHashMap<String, Peer> processResponse(Map m) {
		...
		if (m != null) {
			if (m.containsKey("failure reason")) {
				this.fireUpdateFailed(
						0,
						"The tracker returns the following error message:"
								+ "\t'"
								+ new String((byte[]) m.get("failure reason"))
								+ "'");
				return null;
			} else {
				...
				ArrayList<Map<String, ?>> peerList = new ArrayList<Map<String, ?>>();
				l = new LinkedHashMap<String, Peer>();
				if (peers instanceof List) {
					peerList.addAll((List<Map<String, ?>>) peers);
					if (peerList != null && peerList.size() > 0) {
						for (int i = 0; i < peerList.size(); i++) {
							String peerID = new String(
									(byte[]) (peerList.get(i)).get("peer_id"));
							String ipAddress = new String(
									(byte[]) (peerList.get(i)).get("ip"));
							int port = ((Long) (peerList.get(i)).get("port"))
									.intValue();
    ...
    public synchronized Map contactTracker(byte[] id,
        ...
        try {
            ...
            System.out.println("Contacting tracker... (" + source + ")");
    ...
    /**
     * Sends a message to all listeners with a HashMap containing the list of all
