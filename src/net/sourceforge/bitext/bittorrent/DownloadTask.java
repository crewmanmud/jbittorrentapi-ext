...
public class DownloadTask extends Thread implements IncomingListener,
    ...
    public DownloadTask(Peer peer, byte[] fileID, byte[] myID, boolean init, byte[] bitfield,
        ...
        this.setName("DownloadTask-" + this.peer);
    }
    ...
    public void initConnection() throws UnknownHostException, IOException {
        ...
        this.messageSender = new MessageSender(this.peer.toString(), this.os);
        this.messageSender.addOutgoingListener(this);
        ...
        if (this.initiate) {
            this.messageSender.addMessageToQueue(new Message_HS(this.fileID, this.myID));
    ...
    public void run() {
        try {
            ...
            this.fireTaskCompleted(this.peer.toString(),
                                   DownloadTask.CONNECTION_REFUSED);
    ...
    public synchronized void messageReceived(Message m) {
        if (m == null) {
            this.fireTaskCompleted(this.peer.toString(),
                                   DownloadTask.MALFORMED_MESSAGE);
            return;
        }
        ...
        if (m.getType() == PeerProtocol.HANDSHAKE) {
            ...
            if (Utils.bytesCompare(hs.getFileID(),
                ...
                this.messageSender.addMessageToQueue(new Message_PP(PeerProtocol.BITFIELD,
                        this.bitfield));
            ...
            case PeerProtocol.BITFIELD:
            	...
            	if (message.getPayload() == null) {
            		System.err.println(this.getClass().getName() + "PeerProtocol.BITFIELD NULL payload...");
            		break;
            	}
    ...
    private synchronized void changeState(int newState) {
        ...
        case DOWNLOADING:
            ...
            if (offset >= downloadPiece.getLength()) {
                ...
                messageSender.addMessageToQueue(new Message_PP(PeerProtocol.REQUEST,
