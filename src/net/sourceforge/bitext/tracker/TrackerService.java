...
public class TrackerService extends Service {
	...
	// Default interval in seconds... 300 = 5mins
	private static final int DEFAULT_INTERVAL = 300;
	// Give peers a 60 second grace for updates...
	private static final int INTERVAL_PADDING = 60;
    ...
    public void process(Request req, Response resp) throws IOException {
        HashMap<String, String> hm = this.parseURI(req.getURI());
        byte[] answer = null;
        if(hm != null){
            TreeMap<String, String> param = new TreeMap<String, String>(hm);

            if (param.get("ip") == null) {
                param.put("ip", req.getInetAddress().toString().substring(1));
            }

            ArrayList<Element> peers = new ArrayList<Element>();
     ...
     */
    private HashMap<String, String> parseURI(String uri) throws UnsupportedEncodingException{
     ...
     */
    private byte[] createAnswer(final int err, final List<Element> l) {
        TreeMap ans = new TreeMap();
        try {
            switch (err) {
            case 0:
                ans.put("interval", new Integer(DEFAULT_INTERVAL));
            ...
            default:
                ans.put("failure reason", TrackerService.MESSAGE[err - 1]);
                return BEncoder.encode(ans);
            }
        } catch (Exception e) {
    ...
    private void cleanPeerList(Element root) {
        for (Iterator it = root.getDescendants(new ElementFilter("peer"));
            ...
            if (System.currentTimeMillis() - l > ((DEFAULT_INTERVAL + INTERVAL_PADDING) * 1000)) {
                it.remove();
            }
        }
    }
     ...
     */
    private List<Element> peerSharing(Element root, String hash, String peerid) {
    	ArrayList<Element> peers = new ArrayList<Element>();
     ...
     */
    private int processPeerList(Map param, List<Element> peers) {
        SAXBuilder sb = new SAXBuilder();
        try {
            String hashRaw = (String) param.get("info_hash");
            if (hashRaw == null) {
                return TrackerService.MISSING_FILEID;
            }
            ...
            if (peeridRaw == null) {
                if(param.get("no_peer_id") != null)
                    ...
                    return TrackerService.MISSING_PEERID;
            }
            String port = (String) param.get("port");
            if (port == null) {
                return TrackerService.MISSING_PORT;
            }
            ...
            if (!this.torrentExists(hash)) {
                return TrackerService.UNKNOWN_TORRENT;
            }
            File xmlPeer = getPeerList();
        ...
        return TrackerService.INTERNAL_ERROR;
    }
    
    private static File getPeerList() throws IOException {
    	File xmlPeer = new File((String) Constants.get("peerXML"));
        if (!xmlPeer.exists() || xmlPeer.length() == 0) {
            clearPeerList(xmlPeer);
        }
        return xmlPeer;
    }
    
    public static void clearPeerList() throws IOException {
    	File xmlPeer = new File((String) Constants.get("peerXML"));
    	clearPeerList(xmlPeer);
    }
    
    private static void clearPeerList(File xmlPeer) throws IOException {
