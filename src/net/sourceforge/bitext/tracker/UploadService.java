...
package net.sourceforge.bitext.tracker;
...
public class UploadService extends Service {
    ...
    public final static String[] ERRORS = {"Torrent successfully added to database", "Torrent already in database...", 
    	"This file is not XML valid", "Missing parameters name or file", "File doesn't match the required DTD"};
    ...
    public void process(Request req, Response resp) throws IOException {
        ...
        String hash = Utils.byteArrayToByteString(Utils.hash(
                info));
        int res = UploadService.registerTorrent(name, infoFile, comment, hash);
        if (res == 0) {
     ...
     */
    private static boolean isNewTorrent(Element root, String hash) {
     ...
     */
    private static void addTorrent(Element root, String name, String infoFile,
     ...
     * @throws IOException
     */
    private static void createEmptyTrackerXML(File xmlTorrent) throws IOException {
    ...
    public static void clearTorrents() throws IOException {
    	File xmlTorrent = new File((String) Constants.get("torrentXML"));
    	createEmptyTrackerXML(xmlTorrent);
    }
     ...
     */
    public static int registerTorrent(String name, String infoFile, String comment,
                               String hash) {
        ...
        try {
            ...
            Document torrents;
            try {
                 torrents = getTorrents(xmlTorrent);
            } catch (JDOMException je) {
                return 2;
            }

            Element root = torrents.getRootElement();

            if (root == null || !root.getName().matches("torrentlist")) {
                return 4;
            }

            if (!UploadService.isNewTorrent(root, hash)) {
                return 1;
            }

            UploadService.addTorrent(root, name, infoFile, comment, hash);
            writeXML(xmlTorrent, torrents);
    ...
    private static void writeXML(File xmlTorrent, Document doc) throws IOException {
    ...
    private static Document getTorrents(File xmlTorrent) throws JDOMException, IOException {
    	SAXBuilder sb = new SAXBuilder();

        if (!xmlTorrent.exists() || xmlTorrent.length() == 0) {
        	System.out.println("No torrent currently in DB, creating new DB");
            UploadService.createEmptyTrackerXML(xmlTorrent);
        }
        return sb.build(xmlTorrent);
    }
    
    public static boolean torrentExists(final String hash) throws JDOMException, IOException {
    	File xmlTorrent = new File((String) Constants.get("torrentXML"));
    	Document torrents = getTorrents(xmlTorrent);
		return !isNewTorrent(torrents.getRootElement(), hash);
    }
    
    public static boolean unregisterTorrent(String hash) throws JDOMException, IOException {
    	File xmlTorrent = new File((String) Constants.get("torrentXML"));
    	Document torrents = getTorrents(xmlTorrent);
    	Element root = torrents.getRootElement();
    	
    	if (isNewTorrent(root, hash))
    		return false;
    	
    	for (Object e : root.getChildren()) {
    		Element element = (Element) e;
    		if (element.getChild("hash").getText().equals(hash)) {
    			if (root.removeContent(element)) {
    				writeXML(xmlTorrent, torrents);
    				return true;
    			}
    		}
